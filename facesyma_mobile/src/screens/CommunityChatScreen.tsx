// src/screens/CommunityChatScreen.tsx
...
import { CommunityChatAPI, CommunityMessage } from '../services/api';
import type { ScreenProps } from '../navigation/types';

const MAX_LEN = 2000;

const CommunityChatScreen: React.FC<ScreenProps<'CommunityChat'>> = ({ navigation, route }) => {
  ...
  const flatRef = useRef<FlatList<CommunityMessage>>(null);

  // 18+ yaş kontrolü
  const birthYear = user?.birth_year as number | undefined;
  const currentYear = new Date().getFullYear();
  const isAdult = !birthYear || (currentYear - birthYear >= 18);

  const loadHistory = useCallback(async (before?: number) => {
    try {
      const res = await CommunityChatAPI.getMessages(communityId, before, 50);
      if (before) {
  ...
  useEffect(() => {
    loadHistory();
    CommunityChatAPI.markRead(communityId).catch(() => {});
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      if (!token) return;
      const url = CommunityChatAPI.getWsUrl(communityId, token);
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (data.type === 'new_message' && data.message) {
            const msg: CommunityMessage = data.message;
            setMessages(prev => {
              if (prev.find(m => m._id === msg._id)) return prev;
              return [...prev, msg];
            });
            setTimeout(() => flatRef.current?.scrollToEnd({ animated: true }), 100);
          } else if (data.type === 'error' && data.code === 'age_restricted') {
            Alert.alert(t('community.chat_locked_age', lang), t('community.chat_age_desc', lang));
          }
        } catch {}
      };
    };
  ...
  const sendMessage = async () => {
    ...
    try {
      if (wsRef.current?.readyState === WebSocket.OPEN) {
        ...
        const res = await CommunityChatAPI.sendMessage(communityId, content);
  ...
  const renderItem = ({ item }: { item: CommunityMessage }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onLongPress={() => {
          if (!isMine) {
            Alert.alert(item.sender_username, '', [
              {
                text: t('community.message_privately', lang),
                onPress: () => navigation.navigate('PeerChatRequest', {
                  toUserId: item.sender_id,
                  toUsername: item.sender_username,
        ...
        <View style={[styles.msgRow, isMine && styles.msgRowMine]}>
          {!isMine && (
            <View style={styles.avatar}>
              <Text style={styles.avatarTxt}>{(item.sender_username[0] ?? '?').toUpperCase()}</Text>
            </View>
          )}
    ...
    return (
      <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        <View style={styles.lockBox}>
          ...
          <Text style={styles.lockTitle}>{t('community.chat_locked_age', lang)}</Text>
          <Text style={styles.lockDesc}>{t('community.chat_age_desc', lang)}</Text>
        </View>
      </View>
    );
  }
    ...
    >
      ...
      <View style={[styles.inputRow, { paddingBottom: insets.bottom + spacing.sm }]}>
        <TextInput
          ...
          placeholder={t('community.type_message', lang)}
        ...
        >
          {sending ? (
            ...
            <Text style={styles.sendTxt}>{t('community.chat_send', lang)}</Text>
          )}
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  emptyTxt:     { fontSize: 40, marginBottom: spacing.md },
  listContent:  { padding: spacing.md, gap: spacing.sm },
  msgRow:       { flexDirection: 'row', alignItems: 'flex-end', gap: spacing.sm, marginVertical: 2 },
  msgRowMine:   { flexDirection: 'row-reverse' },
  avatar:       { width: 32, height: 32, borderRadius: 16, backgroundColor: colors.goldDark, alignItems: 'center', justifyContent: 'center' },
  ...
  senderName:   { ...typography.caption, color: colors.gold, fontSize: 11, fontWeight: '700' },
  ...
  charCount:   { ...typography.caption, color: colors.textMuted, fontSize: 10, alignSelf: 'flex-end', marginBottom: 6 },
