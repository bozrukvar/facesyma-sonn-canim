...
# ── DB bağlantıları ────────────────────────────────────────────────────────────
def get_backup():
    return MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)[BACKUP_DB]

def get_source():
    return MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)[SOURCE_DB]
...
async def create_profile(
    ...
    astro_data = {}
    if body.birth_date:
        astro_data = _calculate_basic_astrology(body.birth_date, body.birth_time)
    ...
    result = col.update_one(
        {"user_id": user_id},
        {"$set": profile, "$setOnInsert": {"created_at": datetime.now().isoformat()}},
        upsert=True,
    )
...
async def coach_analyze(body: AnalyzeRequest):
    """
    ...
    """
    lang = body.lang if body.lang in ALL_LANGS else "en"
    db   = get_backup()
    col  = db[f"coach_attributes_{lang}"]

    # Analiz sonucundaki attribute'lardan dominant sıfatları çıkar
    attrs = body.analysis_result.get("attributes", [])
    dominant = [
        a["name"] for a in sorted(attrs, key=lambda x: x.get("score", 0), reverse=True)
        if a.get("name")
    ][:5]
    ...
    # Her sıfat için koç modülü verilerini topla
    coach_output = {}
    for mod in body.include_modules:
        mod_data = []
        for sifat in dominant:
            doc = col.find_one({"_id": sifat}, {mod: 1, "_id": 0})
            if doc and mod in doc:
                mod_data.append({
                    "sifat": sifat,
                    "data":  doc[mod],
                })
    ...
    # Ek analiz verileri
    golden = body.analysis_result.get("golden_ratio", 0)
    face_type = body.analysis_result.get("face_type", "")
...
async def coach_giyim(body: GiyimRequest):
    ...
    # Giyim verisi topla — ilk dominant sıfatın verisi base
    giyim_base = None
    for sifat in dominant:
        doc = col.find_one({"_id": sifat}, {"giyim": 1, "_id": 0})
        if doc and "giyim" in doc and doc["giyim"]:
    ...
    mevsim_onerileri = giyim_base.get("mevsim", {})
    if body.mevsim and body.mevsim in mevsim_onerileri:
        filtered = {body.mevsim: mevsim_onerileri[body.mevsim]}
        if body.kategori and body.kategori in filtered[body.mevsim]:
            filtered[body.mevsim] = {body.kategori: filtered[body.mevsim][body.kategori]}
        mevsim_onerileri = filtered

    # Cinsiyete göre parça değiştir
    gender = body.analysis_result.get("gender", "").lower()
    if gender == "erkek":
        gender_replacements = {
            "elbise": "pantolon",
            "bluz": "gömlek",
        ...
        for mevsim_data in mevsim_onerileri.values():
            for kategori_data in mevsim_data.values():
                if isinstance(kategori_data, dict) and "parca" in kategori_data:
                    ...
                    for i, parca in enumerate(kategori_data["parca"]):
                        for original, replacement in gender_replacements.items():
                            if original.lower() in parca.lower():
                                kategori_data["parca"][i] = parca.replace(original, replacement)
                                break

    # Yüz şekli
    face_type = body.analysis_result.get("face_type", "oval")
    yuz_notu = giyim_base.get("yuz_sekli_notu", {}).get(face_type, "")

    return {
        "dominant_sifatlar": dominant,
        "stil_profili": giyim_base.get("stil_tipi", ""),
        "coaching": giyim_base.get("coaching", {}),
        "yuz_tipi": face_type,
        "lang": lang,
        "renk_paleti": giyim_base.get("renk_paleti", {}),
...
async def birth_analysis(
    ...
    user_id  = get_user_id(authorization)
    lang     = body.lang if body.lang in ALL_LANGS else "en"
    astro    = _calculate_basic_astrology(body.birth_date, body.birth_time, lang)
    numerology = _calculate_numerology(body.birth_date, lang)
...
async def get_goals(user_id: int, status: Optional[str] = None):
    db    = get_backup()
    query = {"user_id": user_id}
    if status:
        query["status"] = status
    goals = list(db["coach_goals"].find(query, {"_id": 1, "title": 1,
        "module": 1, "status": 1, "target_date": 1, "priority": 1}))
    for g in goals:
        g["id"] = str(g.pop("_id"))
    return {"goals": goals}
...
def _calculate_basic_astrology(birth_date_str: str, birth_time_str: Optional[str] = None, lang: str = "tr") -> dict:
    ...
    try:
        ...
        return {"error": error_msgs.get(lang, error_msgs["en"])}

    sun_sign = _get_sun_sign(bd.month, bd.day, lang)
    ...
    result = {
        ...
        "birth_day":     bd.day,
        "birth_year":    bd.year,
        "season":        _get_season(bd.month, lang),
    ...
    if birth_time_str:
        try:
            ...
            result["birth_hour"]     = bt.hour
            result["time_energy"]    = _get_time_energy(bt.hour, lang)
            hints = {
            ...
            result["rising_hint"]    = hints.get(lang, hints["en"])
        except ValueError:
            pass
...
def _get_sun_sign(month: int, day: int, lang: str = "tr") -> str:
    ...
    # Zodiac signs mapped to 18 languages
    zodiac_names = {
        ...
        "ur": ["دلو","مچھلی","مेش","ثور","جوزا","سرطان","اسد","کنیا","میزان","عقرب","قوس","جدی"],
    ...
    for (sm,sd),(em,ed),idx in signs_dates:
        if (month == sm and day >= sd) or (month == em and day <= ed):
            names = zodiac_names.get(lang, zodiac_names["en"])
            return names[idx]
...
def _get_element(sign_idx: int, lang: str = "tr") -> str:
    ...
    elem_dict = elements.get(lang, elements["en"])
...
def _get_quality(sign_idx: int, lang: str = "tr") -> str:
    ...
    qual_dict = qualities.get(lang, qualities["en"])
...
def _get_season(month: int, lang: str = "tr") -> str:
    ...
    season_dict = seasons.get(lang, seasons["en"])
...
def _get_time_energy(hour: int, lang: str = "tr") -> str:
    ...
    energies = {
        "tr": {
            (1,6): "Sezgisel ve manevi",
            (7,11): "Yaratıcı ve başlangıç",
            (12,17): "Sosyal ve işbirlikçi",
            (18,23): "Analitik ve planlayıcı",
            (0,0): "Dönüşümsel"
        },
        "en": {
            (1,6): "Intuitive and spiritual",
            (7,11): "Creative and initiating",
            (12,17): "Social and collaborative",
            (18,23): "Analytical and planning",
            (0,0): "Transformative"
        },
        "de": {
            (1,6): "Intuitiv und spirituell",
            (7,11): "Kreativ und initiativ",
            (12,17): "Sozial und kooperativ",
            (18,23): "Analytisch und planend",
            (0,0): "Transformativ"
        },
        "ru": {
            (1,6): "Интуитивный и духовный",
            (7,11): "Творческий и инициативный",
            (12,17): "Социальный и совместный",
            (18,23): "Аналитический и плановый",
            (0,0): "Трансформативный"
        },
        "ar": {
            (1,6): "بديهي وروحاني",
            (7,11): "إبداعي وريادي",
            (12,17): "اجتماعي وتعاوني",
            (18,23): "تحليلي وتخطيطي",
            (0,0): "تحويلي"
        },
        "es": {
            (1,6): "Intuitivo y espiritual",
            (7,11): "Creativo e iniciador",
            (12,17): "Social y colaborativo",
            (18,23): "Analítico y planificador",
            (0,0): "Transformador"
        },
        "ko": {
            (1,6): "직관적이고 영적",
        ...
        "hi": {
            (1,6): "सहज और आध्यात्मिक",
            (7,11): "रचनात्मक और पहलकदमी",
            (12,17): "सामाजिक और सहयोगी",
            (18,23): "विश्लेषणात्मक और योजनाकार",
            (0,0): "रूपांतरकारी"
        },
        "fr": {
            (1,6): "Intuitif et spirituel",
            (7,11): "Créatif et initiateur",
            (12,17): "Social et collaboratif",
            (18,23): "Analytique et planificateur",
            (0,0): "Transformatif"
        },
        "pt": {
            (1,6): "Intuitivo e espiritual",
            (7,11): "Criativo e iniciador",
            (12,17): "Social e colaborativo",
            (18,23): "Analítico e planejador",
            (0,0): "Transformador"
        },
        "bn": {
            (1,6): "স্বজ্ঞাত এবং আধ্যাত্মিক",
            (7,11): "সৃজনশীল এবং উদ্যোক্তা",
            (12,17): "সামাজिक এবং সহযোগী",
            (18,23): "বিশ্লেষণাত্মক এবং পরিকল্পনাকারী",
            (0,0): "রূপান্তরকারী"
        },
        "id": {
            (1,6): "Intuitif dan spiritual",
            (7,11): "Kreatif dan inisiatif",
            (12,17): "Sosial dan kolaboratif",
            (18,23): "Analitik dan perencanaan",
            (0,0): "Transformatif"
        },
        "ur": {
            (1,6): "شہادت اور روحانی",
            (7,11): "تخلیقی اور ابتدائی",
            (12,17): "سماجی اور تعاونی",
            (18,23): "تجزیاتی اور منصوبہ بندی",
            (0,0): "تبدیلی کا"
        },
        "it": {
            (1,6): "Intuitivo e spirituale",
            (7,11): "Creativo e iniziatore",
            (12,17): "Sociale e collaborativo",
            (18,23): "Analitico e pianificatore",
            (0,0): "Trasformativo"
        },
        "vi": {
            (1,6): "Trực giác và tâm linh",
            (7,11): "Sáng tạo và sáng kiến",
            (12,17): "Xã hội và hợp tác",
            (18,23): "Phân tích và lập kế hoạch",
            (0,0): "Biến đổi"
        },
        "pl": {
            (1,6): "Intuicyjny i duchowy",
            (7,11): "Kreatywny i inicjujący",
            (12,17): "Społeczny i współpracujący",
            (18,23): "Analityczny i planujący",
            (0,0): "Transformacyjny"
        },
    }

    energy_dict = energies.get(lang, energies["en"])
    if 1 <= hour <= 6:
...
def _calculate_numerology(birth_date_str: str, lang: str = "tr") -> dict:
    ...
    meanings = {
        "tr": {
            1:"Lider, öncü, bağımsız",  2:"Diplomat, uyumlu, destekleyici",
            3:"Yaratıcı, ifadeci, sosyal", 4:"Pratik, güvenilir, disiplinli",
            5:"Özgürlükçü, maceracı, uyumlu", 6:"Bakıcı, sorumluluk sahibi, uyumlu",
            7:"Analist, mistik, içe dönük", 8:"Güç, maddi başarı, otorite",
            9:"İnsancıl, idealist, yaratıcı", 11:"Sezgisel usta",
            22:"Mimar usta", 33:"Şifacı usta",
        },
        "en": {
            1:"Leader, pioneer, independent",  2:"Diplomat, harmonious, supportive",
            3:"Creative, expressive, social", 4:"Practical, reliable, disciplined",
            5:"Freedom-loving, adventurous, adaptable", 6:"Caretaker, responsible, harmonious",
            7:"Analyst, mystic, introspective", 8:"Power, material success, authority",
            9:"Humanitarian, idealistic, creative", 11:"Intuitive master",
            22:"Architect master", 33:"Healer master",
        },
        "de": {
            1:"Anführer, Pionier, unabhängig",  2:"Diplomat, harmonisch, unterstützend",
            3:"Kreativ, ausdrucksstark, sozial", 4:"Praktisch, zuverlässig, diszipliniert",
            5:"Freiheitsliebend, abenteuerlustig, anpassungsfähig", 6:"Pfleger, verantwortungsvoll, harmonisch",
            7:"Analytiker, mystisch, introvertiert", 8:"Macht, materieller Erfolg, Autorität",
            9:"Humanitär, idealistisch, kreativ", 11:"Intuitiver Meister",
            22:"Architektur-Meister", 33:"Heiler-Meister",
        },
        "ru": {
            1:"Лидер, первопроходец, независимый",  2:"Дипломат, гармоничный, поддерживающий",
            3:"Творческий, выразительный, социальный", 4:"Практичный, надежный, дисциплинированный",
            5:"Свободолюбивый, авантюрный, адаптивный", 6:"Опекун, ответственный, гармоничный",
            7:"Аналитик, мистический, интровертный", 8:"Власть, материальный успех, авторитет",
            9:"Гуманитарный, идеалистический, творческий", 11:"Интуитивный мастер",
            22:"Архитектор-мастер", 33:"Целитель-мастер",
        },
        "ar": {
            1:"قائد، رائد، مستقل",  2:"دبلوماسي، متناغم، داعم",
            3:"مبدع، تعبيري، اجتماعي", 4:"عملي، موثوق، منضبط",
            5:"محب للحرية، مغامر، متكيف", 6:"مقدم رعاية، مسؤول، متناغم",
            7:"محلل، غامض، منطوٍ", 8:"القوة، النجاح المادي، السلطة",
            9:"إنساني، مثالي، مبدع", 11:"معلم حدسي",
            22:"معلم معماري", 33:"معلم شاف",
        },
        "es": {
            1:"Líder, pionero, independiente",  2:"Diplomático, armónico, de apoyo",
            3:"Creativo, expresivo, social", 4:"Práctico, confiable, disciplinado",
            5:"Amante de la libertad, aventurero, adaptable", 6:"Cuidador, responsable, armónico",
            7:"Analista, místico, introvertido", 8:"Poder, éxito material, autoridad",
            9:"Humanitario, idealista, creativo", 11:"Maestro intuitivo",
            22:"Maestro arquitecto", 33:"Maestro sanador",
        },
        "ko": {
            1:"지도자, 개척자, 독립적",  2:"외교관, 조화로운, 지지적",
            3:"창의적, 표현적, 사회적", 4:"실용적, 신뢰할 수 있는, 규율 있는",
            5:"자유를 사랑하는, 모험적, 적응 가능", 6:"간병인, 책임 있는, 조화로운",
            7:"분석가, 신비로운, 내향적", 8:"권력, 물질적 성공, 권위",
            9:"인도주의적, 이상주의적, 창의적", 11:"직관적 마스터",
            22:"건축 마스터", 33:"치유 마스터",
        },
        "ja": {
            1:"指導者、開拓者、独立的",  2:"外交官、調和的、支援的",
            3:"創意的、表現的、社交的", 4:"実用的、信頼できる、規律的",
            5:"自由を愛する、冒険心豊か、適応可能", 6:"介護者、責任のある、調和的",
            7:"分析家、神秘的、内向的", 8:"力、物質的成功、権威",
            9:"人道的、理想主義的、創意的", 11:"直感的マスター",
            22:"建築マスター", 33:"ヒーラーマスター",
        },
        "zh": {
            1:"领导者、先驱、独立",  2:"外交官、和谐、支持",
            3:"创意、表达、社交", 4:"实用、可靠、纪律",
            5:"爱自由、冒险、适应", 6:"照顾者、负责、和谐",
            7:"分析家、神秘、内向", 8:"力量、物质成功、权威",
            9:"人道、理想、创意", 11:"直觉大师",
            22:"建筑大师", 33:"治疗大师",
        },
        "hi": {
            1:"नेता, अग्रदूत, स्वतंत्र",  2:"कूटनीतिज्ञ, सामंजस्यपूर्ण, सहायक",
            3:"रचनात्मक, अभिव्यक्तिपूर्ण, सामाजिक", 4:"व्यावहारिक, विश्वसनीय, अनुशासित",
            5:"स्वतंत्रता प्रेमी, साहसी, अनुकूलनीय", 6:"देखभालकर्ता, जिम्मेदार, सामंजस्यपूर्ण",
            7:"विश्लेषक, रहस्यमय, अंतर्मुखी", 8:"शक्ति, भौतिक सफलता, अधिकार",
            9:"मानवतावादी, आदर्शवादी, रचनात्मक", 11:"अंतर्ज्ञानी मास्टर",
            22:"आर्किटेक्ट मास्टर", 33:"हीलर मास्टर",
        },
        "fr": {
            1:"Leader, pionnier, indépendant",  2:"Diplomate, harmonieux, solidaire",
            3:"Créatif, expressif, social", 4:"Pratique, fiable, discipliné",
            5:"Libertaire, aventurier, adaptable", 6:"Soignant, responsable, harmonieux",
            7:"Analyste, mystique, introverti", 8:"Pouvoir, succès matériel, autorité",
            9:"Humanitaire, idéaliste, créatif", 11:"Maître intuitif",
            22:"Maître architecte", 33:"Maître guérisseur",
        },
        "pt": {
            1:"Líder, pioneiro, independente",  2:"Diplomata, harmônico, solidário",
            3:"Criativo, expressivo, social", 4:"Prático, confiável, disciplinado",
            5:"Amante da liberdade, aventureiro, adaptável", 6:"Cuidador, responsável, harmônico",
            7:"Analista, místico, introvertido", 8:"Poder, sucesso material, autoridade",
            9:"Humanitário, idealista, criativo", 11:"Mestre intuitivo",
            22:"Mestre arquiteto", 33:"Mestre curador",
        },
        "bn": {
            1:"নেতা, অগ্রগামী, স্বাধীন",  2:"কূটনীতিজ্ঞ, সামঞ্জস্যপূর্ণ, সহায়ক",
            3:"সৃজনশীল, প্রকাশমূলক, সামাজিক", 4:"ব্যবহারিক, নির্ভরযোগ্য, শৃঙ্খলাবদ্ধ",
            5:"স্বাধীনতা প্রেমী, দুঃসাহসী, অভিযোজিত", 6:"যত্নশীল, দায়বদ্ধ, সামঞ্জস্যপূর্ণ",
            7:"বিশ্লেষক, রহস্যময়, অন্তর্মুখী", 8:"শক্তি, বস্তুগত সাফল্য, কর্তৃপক্ষ",
            9:"মানবিক, আদর্শবাদী, সৃজনশীল", 11:"স্বজ্ঞাত মাস্টার",
            22:"স্থপতি মাস্টার", 33:"নিরাময়কারী মাস্টার",
        },
        "id": {
            1:"Pemimpin, pelopor, independen",  2:"Diplomat, harmonis, suportif",
            3:"Kreatif, ekspresif, sosial", 4:"Praktis, dapat diandalkan, disiplin",
            5:"Pecinta kebebasan, petualang, adaptif", 6:"Penasehat, bertanggung jawab, harmonis",
            7:"Analis, mistis, introvert", 8:"Kekuatan, kesuksesan material, otoritas",
            9:"Humaniter, idealis, kreatif", 11:"Ahli intuitif",
            22:"Ahli arsitektur", 33:"Ahli penyembuh",
        },
        "ur": {
            1:"رہنما، علمبردار، خود مختار",  2:"سفارتکار، ہماہنگ، معاون",
            3:"تخلیقی، اظہاری، سماجی", 4:"عملی، قابل اعتماد، نظم و ضبط",
            5:"آزادی سے محبت، مہم جو، موافق", 6:"دیکھ بھال کار، ذمہ دار، ہماہنگ",
            7:"تجزیہ کار، غامض، باطن پسند", 8:"طاقت، مادی کامیابی، اقتدار",
            9:"انسان دوست، نظریہ پرست، تخلیقی", 11:"شہادت رکھنے والا ماہر",
            22:"معماری ماہر", 33:"شفاء دینے والا ماہر",
        },
        "it": {
            1:"Leader, pioniere, indipendente",  2:"Diplomatico, armonico, solidale",
            3:"Creativo, espressivo, sociale", 4:"Pratico, affidabile, disciplinato",
            5:"Amante della libertà, avventuriero, adattabile", 6:"Caregiver, responsabile, armonico",
            7:"Analista, mistico, introverso", 8:"Potere, successo materiale, autorità",
            9:"Umanitario, idealista, creativo", 11:"Maestro intuitivo",
            22:"Maestro architetto", 33:"Maestro guaritore",
        },
        "vi": {
            1:"Lãnh đạo, tiên phong, độc lập",  2:"Ngoại giao, hài hòa, hỗ trợ",
            3:"Sáng tạo, biểu hiện, xã hội", 4:"Thực tế, đáng tin cậy, kỷ luật",
            5:"Yêu thích tự do, mạo hiểm, thích ứng", 6:"Chăm sóc, có trách nhiệm, hài hòa",
            7:"Nhà phân tích, bí ẩn, hướng nội", 8:"Sức mạnh, thành công vật chất, quyền hạn",
            9:"Nhân đạo, lý tưởng, sáng tạo", 11:"Bậc thầy trực giác",
            22:"Bậc thầy kiến trúc", 33:"Bậc thầy chữa bệnh",
        },
        "pl": {
            1:"Lider, pionier, niezależny",  2:"Dyplomata, harmonijny, wspierający",
            3:"Kreatywny, ekspresyjny, społeczny", 4:"Praktyczny, niezawodny, zdyscyplinowany",
            5:"Miłośnik wolności, awanturnik, adaptacyjny", 6:"Opiekun, odpowiedzialny, harmonijny",
            7:"Analityk, mistyczny, introwertyzm", 8:"Moc, sukcesy materialne, autorytet",
            9:"Humanitarny, idealistyczny, kreatywny", 11:"Mistrz intuicji",
            22:"Mistrz architektury", 33:"Mistrz uzdrawiania",
        },
    }

    meaning_dict = meanings.get(lang, meanings["en"])
