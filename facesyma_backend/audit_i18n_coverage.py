...
_RE_TRANS_TAG   = re.compile(r'{% trans ')
_RE_TRANS_DICTS = re.compile(r'[A-Z_]+_STRINGS\s*=\s*{')
_RE_TRANS_CALL  = re.compile(r"_\('")
_RE_MSGSTR      = re.compile(r'^msgstr "', re.MULTILINE)
_RE_MSGSTR_EMPTY = re.compile(r'^msgstr ""$', re.MULTILINE)
...
class I18nAudit:
    ...
    def check_template_files(self):
        ...
        for tmpl_file in template_files:
            try:
                ...
                # Count trans tags
                trans_count = len(_RE_TRANS_TAG.findall(content))
                if trans_count > 0:
                    with_tags += 1
                else:
        ...
        _st = self.stats
        _st['template_files_checked'] = len(template_files)
        _st['templates_with_i18n'] = with_tags
        _st['templates_missing_i18n'] = missing_tags
    ...
    def check_translation_service(self):
        ...
        dict_count = len(_RE_TRANS_DICTS.findall(content))
        string_count = len(_RE_TRANS_CALL.findall(content))
    ...
    def check_locale_files(self):
        ...
        if not locale_dir.exists():
            _issues['missing_locale_dir'].append(str(locale_dir))
        ...
        for code, name in languages:
            ...
            _po_exists = po_file.exists()
            _mo_exists = mo_file.exists()
            if not _po_exists:
                missing_po.append(code)
            if not _mo_exists:
                missing_mo.append(code)
            if _po_exists and _mo_exists:
                complete += 1
        ...
        if missing_po:
            _issues['missing_po_files'].append(missing_po)
            print(f"  ❌ Missing .po files: {missing_po}")

        if missing_mo:
            _issues['missing_mo_files'].append(missing_mo)
        ...
        if pot_file.exists():
            ...
            _issues['missing_pot'].append(str(pot_file))
    ...
    def check_translation_completeness(self):
        ...
        for lang_dir in sorted(locale_dir.iterdir()):
            ...
            total_msgstr = len(_RE_MSGSTR.findall(content))
            empty_msgstr = len(_RE_MSGSTR_EMPTY.findall(content))
            translated = total_msgstr - empty_msgstr - 1  # -1 for header

            languages_stats[_lname] = {
                'total': total_msgstr,
        ...
        for lang, stats in sorted(languages_stats.items()):
            _st = stats['total']
            if _st > 0:
                _sttr = stats['translated']
                pct = int((_sttr / (_st - 1)) * 100)
                status = "✅" if pct == 100 else "⚠️ " if pct > 50 else "❌"
                print(f"    {status} {lang:10} — {pct:3}% ({_sttr:2}/{_st-1})")
    ...
    def check_api_endpoints(self):
        ...
        print(f"  ✅ {using_i18n}/{_n_views} API view files have i18n")

        if using_i18n < _n_views:
            self.issues['views_without_i18n'].append(_n_views - using_i18n)
    ...
    def generate_report(self):
        ...
        _st = self.stats
        print('\n📊 STATISTICS:')
        for key, value in sorted(_st.items()):
            print(f'  • {key}: {value}')

        if _issues:
            print('\n⚠️  ISSUES FOUND:')
            for issue_type, items in _issues.items():
                print(f'\n  {issue_type}:')
        ...
        if not _issues and _st.get('languages_complete') == _st.get('languages_total'):
